 ...
 */
class OneSDicconst extends \yii\db\ActiveRecord
{
    const TYPE_DROP = 1;
    ...
    {
        $pConst = OneSPconst::findOne(['const_id' => $this->id, 'org' => Yii::$app->user->identity->organization_id]);
    ...
    public static function getSetting($denom)
    {
        ...
        if ($model) {
            $pConst = OneSPconst::findOne(['const_id' => $model->id, 'org' => Yii::$app->user->identity->organization_id]);
            if (!empty($pConst)) {
                return $pConst->value;
            } else {
