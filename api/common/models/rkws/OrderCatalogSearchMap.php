    ...
    public function search($params)
    {
        ...
        $joins = [
            ...
            1 => " LEFT JOIN `$dbName`.`rk_product` fprod ON fmap.serviceproduct_id = fprod.id
                   LEFT JOIN `$dbName`.`rk_storetree` fstore ON fmap.outer_store_id = fstore.id AND fmap.org_id = fstore.acc  AND fstore.type = 2 ",

            2 => " LEFT JOIN `$dbName`.`iiko_product` fprod ON fmap.serviceproduct_id = fprod.id
                   LEFT JOIN `$dbName`.`iiko_store` fstore ON fmap.outer_store_id = fstore.id AND fmap.org_id = fstore.org_id  AND fstore.is_active = 1 ",

            8 => " LEFT JOIN `$dbName`.`one_s_good` fprod ON fmap.serviceproduct_id = fprod.id
                   LEFT JOIN `$dbName`.`one_s_store` fstore ON fmap.outer_store_id = fstore.id AND fmap.org_id = fstore.org_id ",
