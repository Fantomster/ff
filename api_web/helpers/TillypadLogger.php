...
{
    public static $tableName = '{{%tillypad_log}}';
    private static $guide;
    private static $row;

    /**
     * TillypadLogger constructor.
     *
     * @throws \Exception
     */
    ...
    public function setOperation($denom)
    {
        $operation = (new Query())
            ...
            ->where(['service_id' => Registry::TILLYPAD_SERVICE_ID, 'denom' => $denom])
            ->one(\Yii::$app->db_api);
        ...
        if (!empty($operation)) {
            ...
            throw new \Exception(\Yii::t('api_web', 'Not found operation service_id:{id} {denom}', [
                ...
                'id'    => Registry::TILLYPAD_SERVICE_ID,
                'denom' => $denom
            ]), 999);
        }
    }
    ...
    {
        return 'log_service_' . Registry::TILLYPAD_SERVICE_ID;
    }
}