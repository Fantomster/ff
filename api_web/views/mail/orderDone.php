...
/** @var $senderOrg \common\models\Organization */
...
if ($senderIsClient && $self) {
    $text = Yii::t('app', 'common.mail.order_done.client_himself', ['ru' => 'Вы завершили заказ N°{order_id} от {order_date} для поставщика {org_name}.', 'order_id' => $order->id, 'org_name' => $order->vendor->getName(), 'order_date' => $order->getFormattedCreationDate()]);
}
if ($senderIsClient && !$self) {
    $text = Yii::t('app', 'common.mail.order_done.text_for_vendor', ['ru' => 'Ресторан {org_name} завершил заказ N°{order_id} от {order_date}.', 'order_id' => $order->id, 'org_name' => $order->client->getName(), 'order_date' => $order->getFormattedCreationDate()]);
}
if (!$senderIsClient && $self) {
    $text = Yii::t('app', 'common.mail.order_done.vendor_himself', ['ru' => 'Вы завершили заказ N°{order_id} от {order_date} для ресторана {org_name}.', 'order_id' => $order->id, 'org_name' => $order->client->getName(), 'order_date' => $order->getFormattedCreationDate()]);
}
if (!$senderIsClient && !$self) {
    $text = Yii::t('app', 'common.mail.order_done.text_for_client', ['ru' => 'Поставщик {org_name} завершил заказ N°{order_id} от {order_date}.', 'order_id' => $order->id, 'org_name' => $order->vendor->getName(), 'order_date' => $order->getFormattedCreationDate()]);
}
