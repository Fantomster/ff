...
<div style="width:100%;-webkit-border-radius: 3px;border-radius: 3px;background-color: #fff;border-top: 3px solid #00a65a;box-shadow: 0 1px 1px rgba(0,0,0,0.1); width: 570px; margin-top: 10px;">
    <div style="width:100%;height: 100%;">
        ...
        <?php if (!empty($order->comment)) { ?>
            <div style="width: 100%;height: auto;border: 1px solid #DDDDDD;float: left;margin: 30px 0%; border-radius: 100px;">
                <div style="border-right: 1px solid #DDDDDD;width: 55px;float: left;height: 50px;">
                    <img src="https://app.mixcart.ru/img/c.png" style="margin-left: 18px;margin-top: 13px;" alt="">
                </div>
        ...
        <?php if (!empty($deleted)) { ?>
            <h3><?= Yii::t('app', 'common.mail.bill.deleted_caption', ['ru' => 'Удаленные из заказа товары:']) ?></h3>
            <?= $this->render('_deleted-grid', compact('order', 'deleted')) ?>
            <?php
        }
        if (!empty($changed)) {
            ?>
            <h3><?= Yii::t('app', 'common.mail.bill.changed_caption', ['ru' => 'Измененные позиции отмечены зеленым:']) ?></h3>
        <?php } elseif (!empty($deleted)) {
            ?>
            <h3><?= Yii::t('app', 'common.mail.bill.rest_positions', ['ru' => 'Оставшиеся позиции:']) ?></h3>
        <?php } ?>
        <?= $this->render('_view-grid', compact('dataProvider', 'order', 'changed')) ?>
