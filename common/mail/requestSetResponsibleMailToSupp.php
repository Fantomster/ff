		...
		<table cellspacing="0" cellpadding="0" border="0" align="center" bgcolor="#ffffff" width="600" style="margin: auto; border: 5px solid #66BC75;" class="email-container">
			...
			<tr>
				<td style="text-align: center; font-family: 'Open Sans', sans-serif;  mso-height-rule: exactly; color: #555555;">
					<h1 style="margin-bottom: 0;font-size: 16px;line-height: 27px;font-weight: 500;padding-top: 25px"><?= Yii::t('app', 'common.mail.request_set_resp_mail.dear', ['ru'=>'Уважаемый']) ?> <?=$vendor->profile->full_name;?>!</h1>
				</td>
			</tr>
			...
			<tr>
				<td style="padding: 10px 30px; text-align: center; font-family: 'Open Sans', sans-serif; font-size: 14px; mso-height-rule: exactly; line-height: 20px; color: #555;">
					<b>&laquo;<?=$request->client->name;?>&raquo;</b>  <?= Yii::t('app', 'common.mail.request_set_resp_mail.set_executor', ['ru'=>'назначил Вас исполнителем по заявке']) ?> <b>&laquo;<?=$request->product;?>&raquo;</b>
					<br><br>
			...
			<tr>
				<td style="padding: 10px 30px; padding-bottom: 40px; text-align: center; font-family: 'Open Sans', sans-serif; font-size: 16px; mso-height-rule: exactly; line-height: 16px; color: #555;">
					<a href="<?= Yii::$app->urlManagerFrontend->createAbsoluteUrl(["/request/view", 'id' => $request->id]) ?>" style="background-color: #66BC75; padding: 10px 30px; border-radius: 30px; color: #fff; text-decoration: none; cursor: pointer;"><?= Yii::t('app', 'common.mail.request_set_resp_mail.go_to', ['ru'=>'Перейти к заявке']) ?></a>
					<br><br>
