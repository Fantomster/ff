...
class Organization extends \yii\db\ActiveRecord {
    ...
    public function getRestaurantCategories() {
        ...
        $categories = ArrayHelper::map(Category::find()
                                ...
                                ->asArray()
                                ->all(), 'id', 'name');
        $categories[''] = Yii::t('app', 'common.models.all_categories', ['ru'=>'Все категории']);
        ksort($categories);
        return $categories;
    }
    ...
    public function getSuppliers($category_id = '', $all = false) {
        ...
        $vendors = ArrayHelper::map($query->orderBy(['organization.name' => SORT_ASC])
                                ->asArray()
                                ->all(), 'id', 'name');
        if(!$all) $vendors[''] = Yii::t('app', 'common.models.all_vendors', ['ru'=>'Все поставщики']);
        ksort($vendors);
        return $vendors;
    }
    ...
    public function getClients() {
        ...
        $clients = ArrayHelper::map($query
                                ->asArray()
                                ->all(), 'id', 'name');
        $clients[''] = Yii::t('app', 'common.models.all_rest', ['ru'=>'Все рестораны']);
        ksort($clients);
        return $clients;
    }
