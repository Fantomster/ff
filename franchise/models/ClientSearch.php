...
class ClientSearch extends Organization {
    ...
    public function search($params, $franchisee_id) {
        ...
        $query = "SELECT org.id as id, org.name as name, (select count(id) from relation_supp_rest where rest_org_id=org.id) as clientCount, 
                (select count(id) from relation_supp_rest where rest_org_id=org.id and created_at BETWEEN CURDATE() - INTERVAL 30 DAY AND CURDATE()) as clientCount_prev30, 
                count(ord.id) as orderCount,
                (select count(id) from `order` where client_id=org.id and created_at BETWEEN CURDATE() - INTERVAL 30 DAY AND CURDATE() ) as orderCount_prev30,
                sum(ord.total_price) as orderSum,
                (select sum(total_price) from `order` where client_id=org.id and created_at BETWEEN CURDATE() - INTERVAL 30 DAY AND CURDATE() ) as orderSum_prev30,
