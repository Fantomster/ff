...
<div class="modal-header">
    ...
    <h4 class="modal-title"><?= $catalogBaseGoods->isNewRecord? Yii::t('app', 'Новый товар') : Yii::t('app', 'Корректировка товара') ?></h4>
</div>
<div class="modal-body">
    ...
    <?= $form->field($catalogBaseGoods, 'note')->textarea(['rows' => 3])->label(Yii::t('app', 'Комментарий к товару')) ?>
    
    <?= $catalogBaseGoods->isNewRecord? $form->field($catalogBaseGoods, 'cat_id')->hiddenInput(['value'=> Yii::$app->request->get('id')])->label(false):'' ?>
</div>
<div class="modal-footer">
    <?= Html::button($catalogBaseGoods->isNewRecord ? '<i class="icon fa fa-plus-circle"></i> ' . Yii::t('app', 'Создать') . ' ' : '<i class="icon fa fa-save"></i> ' . Yii::t('app', 'Сохранить') . ' ', ['class' => $catalogBaseGoods->isNewRecord ? 'btn btn-success edit' : 'btn btn-success edit']) ?>
