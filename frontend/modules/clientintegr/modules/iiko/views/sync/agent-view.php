                        ...
                        echo GridView::widget([
                            ...
                            'columns'          => [
                                ...
                                [
                                    ...
                                    'label'          => 'Отсрочка платежей в днях',
                                    'contentOptions' => function ($model) {
                                        return ["id" => "delay" . $model['id']];
                                    },
                                ],
                            ],
                        ]);
...
$url_edit_payment_delay = Url::toRoute('sync/edit-payment-delay');
...
$js = <<< JS
    $(function () {
                ...
                $('#denom_forever').on('click', function () {
                    ...
                    $.post(url_edit_new, {id:selectvalue, number:number}, function (data) {
                        if (data == true) {
                            ...
                            alert('Не удалось сохранить контрагента IIKO.');
                        }
                    })
                });
            });
                    ...
                    if (selectvalue != '') {
                        $.post(url_edit_comment, {comm:selectvalue, number:number}, function (data) {
                            if (data == true) {
                                ...
                                alert('Не удалось сохранить комментарий к контрагенту IIKO.');
                            }
                        })
                    }
                    swal.close();
                ...
                $('#denom_forever').on('click', function () {
                    ...
                    $.post(url_edit_active, {activ:selectvalue, number:number}, function (data) {
                        if (data == true) {
                            ...
                            alert('Не удалось изменить статус активности контрагента IIKO.');
                        }
                    })
                });
            });
        ...
        function links_column_delay () { // реакция на нажатие строки в столбце "Отсрочка платежа в днях"
            $('[data-col-seq='+7+']').each(function() {
                var idtd = $(this).attr('id');
                var idtds = String(idtd);
                var idnumber = idtds.substring(5);
                var idbutton = 'butdel' + idnumber;
                var cont_old = $(this).html();
                var cont_new = '<button class="button-delay" id="'+idbutton+'" style="color:#6ea262;background:none;border:none;border-bottom:1px dashed">'+cont_old+'</button>';
                if (idbutton!='butdelined') {
                    $(this).html(cont_new);
                }
            });
            $('.button-delay').on('click', function () {
                $('a .button-delay').click(function(){ return false;});
                var idbutton = $(this).attr('id');
                var idbuttons = String(idbutton);
                var number = idbuttons.substring(6);   // идентификатор строки
                var tovar = $("#agent"+number).html(); // наименование контрагента
                var cont_old = $(this).html();         // содержание ячейки до форматирования
                var nesopost = '0';   // содержание несопоставленной ячейки
                swal({
                    html: '<span style="font-size:14px">Изменить отсрочку платежа в днях (максимум 365)</span></br></br><span id="tovar">0</span></br></br>' +
                    '<input type="text" id="bukv-tovar" class="swal2-input" placeholder="Введите отсрочку платежа в днях" autofocus>'+
                    '<div id="bukv-tovar2" style="margin-top:0px;padding-top:0px;"></div>'+'<div id="bukv-tovar3" style="margin-top:0px;padding-top:0px;"></div>'
                    +'<div id="bukv-tovar4" style="margin-top:0px;padding-top:0px;"></div>'
                    + '</br><input type="submit" name="denom_forever" id="denom_forever" class="btn btn-sm btn-primary butsubmit" value="Сохранить"> '
                    + '<input type="button" id="denom_close" class="btn btn-sm btn-outline-danger" value="Отменить">',
                ...
                $('#denom_forever').on('click', function () {
                    var selectvalue = $('#bukv-tovar').val();
                    var url_edit_payment_delay = '$url_edit_payment_delay';
                    if (selectvalue != '') {
                        $.post(url_edit_payment_delay, {delay:selectvalue, number:number}, function (data) {
                            if (data == true) {
                                $('#butdel'+number).html(selectvalue);
                            } else {
                                alert('Не удалось сохранить отсрочку платежа к контрагенту IIKO.');
                            }
                        })
                    }
                    swal.close();
