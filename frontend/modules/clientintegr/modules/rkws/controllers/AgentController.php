   ...
   //  $resres = ApiHelper::getAgents();     
    ...
    public function actionAgentAutocomplete($term = null)
    {
        ...
        if (!is_null($term)) {
            ...
            $sql = "SELECT id, `name` as text FROM organization where `name` LIKE '%$term%' and type_id = 2 and deleted = 0 and id in (SELECT supp_org_id FROM fkeeper.relation_supp_rest where rest_org_id = $organisation_id)";
            $db = \Yii::$app->db;
