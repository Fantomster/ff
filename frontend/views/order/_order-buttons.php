...
$btnCancel = Html::button('<span><i class="icon fa fa-ban"></i> ' . Yii::t('message', 'frontend.views.order.cancel_seven', ['ru'=>'Отменить']) . ' </span>', [
            'class' => "btn btn-outline-danger col-md-6 col-sm-6 col-xs-12   cancel-order",
...
$btnConfirm = Html::button('<span><i class="icon fa fa-check"></i> ' . Yii::t('message', 'frontend.views.order.confirm', ['ru'=>'Подтвердить']) . ' </span>', [
    'class' => "btn btn-outline-success col-md-6 col-sm-6 col-xs-12   btnOrderAction",
    'data' => [
        'action' => "confirm",
...
$btnCloseOrder = Html::button('<span><i class="icon fa fa-check"></i> ' . Yii::t('message', 'frontend.views.order.end_all', ['ru'=>'Завершить']) . ' </span>', [
    'class' => (isset($order->vendor->ediOrganization->gln_code) && $order->vendor->ediOrganization->gln_code>0) ? 'btn btn-outline-success col-md-6 col-sm-6 col-xs-12 completeEdi' : 'btn btn-outline-success col-md-6 col-sm-6 col-xs-12 btnOrderAction',
    'data' => $data,
]);
$canEdit = false;
if ($order->isObsolete) {
    ...
    switch ($order->status) {
        case Order::STATUS_AWAITING_ACCEPT_FROM_VENDOR:
            ...
            if ($organizationType == Organization::TYPE_RESTAURANT) {
                $statusInfo .= '<a href="#" class="btn btn-warning col-md-6 col-sm-6 col-xs-12   disabled status"><span class="badge"><i class="icon fa fa-info"></i></span>&nbsp; ' . Yii::t('message', 'frontend.views.order.waiting', ['ru'=>'Ожидаем подтверждения']) . ' </a>';
            } else {
        ...
        case Order::STATUS_AWAITING_ACCEPT_FROM_CLIENT:
            ...
            if ($organizationType == Organization::TYPE_SUPPLIER) {
                $statusInfo .= '<a href="#" class="btn btn-warning col-md-6 col-sm-6 col-xs-12   disabled status"><span class="badge"><i class="icon fa fa-info"></i></span>&nbsp; ' . Yii::t('message', 'frontend.views.order.waiting_two', ['ru'=>'Ожидаем подтверждения']) . ' </a>';
            } else {
                $actionButtons .= $btnConfirm;
            }
            $canEdit = true;
        ...
        case Order::STATUS_PROCESSING:
            ...
            if ($organizationType == Organization::TYPE_SUPPLIER) {
                $statusInfo .= '<a href="#" class="btn btn-processing col-md-6 col-sm-6 col-xs-12   disabled status"><span class="badge"><i class="icon fa fa-info"></i></span>&nbsp; ' . Yii::t('message', 'frontend.views.order.executes', ['ru'=>'Исполняется']) . ' </a>';
            } else {
        ...
        case Order::STATUS_DONE;
            $statusInfo .= '<a href="#" class="btn btn-success col-md-6 col-sm-6 col-xs-12   disabled"><span class="badge"><i class="icon fa fa-info"></i></span>&nbsp; ' . Yii::t('message', 'frontend.views.order.ready', ['ru'=>'Выполнен']) . ' </a>';
            break;
        case Order::STATUS_REJECTED;
            $statusInfo .= '<a href="#" class="btn btn-danger col-md-6 col-sm-6 col-xs-12   disabled"><span class="badge"><i class="icon fa fa-info"></i></span>&nbsp; ' . Yii::t('message', 'frontend.views.order.rejected', ['ru'=>'Отклонен']) . ' </a>';
            break;
        case Order::STATUS_CANCELLED;
            $statusInfo .= '<a href="#" class="btn btn-danger col-md-6 col-sm-6 col-xs-12   disabled"><span class="badge"><i class="icon fa fa-info"></i></span>&nbsp; ' . Yii::t('message', 'frontend.views.order.canceled_two', ['ru'=>'Отменен']) . ' </a>';
            break;
    }
}
...
<div class="box box-info block_wrapper" style="height:auto;">
    <div class="box-header col-xs-12">
        ...
        Html::a('<i class="icon fa fa-print"></i> ' . Yii::t('message', 'frontend.views.order.open_in_pdf', ['ru'=>'Открыть в виде PDF']) . ' ', ['order/pdf', 'id' => $order->id], [
            'class' => 'btn btn-outline-default col-md-6 col-sm-6 col-xs-12   pull-right',
            'target' => '_blank',
            'data-toggle' => 'tooltip',
        ...
        (isset($canRepeatOrder) && $canRepeatOrder) ? Html::a('<i class="icon fa fa-refresh"></i> ' . Yii::t('message', 'frontend.views.order.repeat_order_two', ['ru'=>'Повторить заказ']) . ' ', ['order/repeat', 'id' => $order->id], [
                    'class' => 'btn btn-default col-md-6 col-sm-6 col-xs-12   pull-right'
                ]) : ""
        ?>
        ...
        <?= $canEdit && !$edit ? Html::a('<i class="icon fa fa-save"></i> ' . Yii::t('message', 'frontend.views.order.edit', ['ru'=>'Редактировать']), ['/order/edit', "id" => $order->id], ['class' => 'btn btn-success col-md-6 col-sm-6 col-xs-12   pull-right btnSave']) : "" ?>
<div style="clear: both; height: 5px;"></div>
        ...
        if(isset($licenses['mercury']))
            echo Html::a(' ' . Yii::t('app', 'frontend.views.order.index.mercury', ['ru'=>'Погасить ВСД']). ' ', ['/clientintegr/merc/default'], [
                'class' => 'btn btn-outline-processing col-md-6 col-sm-6 col-xs-12   pull-right',
                'target' => '',
                'data-toggle' => 'tooltip',
        ...
        Html::a('<i class="icon fa fa-file-excel-o"></i> ' . Yii::t('app', 'frontend.views.order.index.report', ['ru'=>'отчет xls']) . ' ', ['order/order-to-xls', 'id' => $order->id], [
            'class' => 'btn btn-outline-success col-md-6 col-sm-6 col-xs-12   pull-right',
            'target' => '',
            'data-toggle' => 'tooltip',
