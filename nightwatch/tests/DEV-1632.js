...
module.exports = {
	before: function (browser) {
		console.log('Begining test DEV-1632...');
	},

	after: function (browser) {
		console.log('Finishing test DEV-1632...');
		browser.end();
	},

	'Test DEV-1632 products operations': function (browser) {
		...
		browser.saveScreenshot('tesssstttt.png');

		function test_product_operation_plus_minus(b, array, clickCount) {

			b.element('css selector', 'body', function () {
				console.log('Testing product plus operation...')
			});
			...
			for (let key in array) {
				...
				b.pause(200);
				b.verify.attributeEquals('#test_input', 'value', String(Number(array[key][2]) * clickCount));
				b.pause(200);
				...
				let str = String(Number(array[key][2]) * clickCount);
				b.verify.attributeEquals('app-order .cart-item:nth-child(' + String(cart_key + 1) + ') input', 'value', str);
			...
			b.element('css selector', 'body', function () {
				console.log('Testing product minus operation...')
			});

			for (let key in array) {
				b.execute(function (key) {
					let a1 = document.querySelector('app-product-list table tbody tr:nth-child(' + key + ') td:nth-child(6) .quantity-minus');
					let a2 = document.querySelector('app-product-list table tbody tr:nth-child(' + key + ') td:nth-child(6) input');
					a1.setAttribute('id', 'test');
					a2.setAttribute('id', 'test_input');
					let a = document.getElementById('test');
		...
		function test_product_operation_input(b, array) {
			b.element('css selector', 'body', function () {
				console.log('Testing random inputs...')
			});
			...
			function expected_input(multiplicity, my_input) {
				...
				if (n === 0) {
					return parseFloat(String(multiplicity));
				}
			...
			for (let key in array) {
				...
				b.setValue('#test_input', [randInput, b.Keys.ENTER]);

				b.pause(1000);

				b.verify.attributeEquals('#test_input', 'value', String((Number(array[key][2]), Number(randInput))));

				b.pause(1000);

				let cart_key = Number(key);

				b.verify.attributeEquals('app-order .cart-item:nth-child(' + String(cart_key + 1) + ') input', 'value', String(expected_input(Number(array[key][2]), Number(randInput))));
		...
		// test_product_operation_plus_minus(browser, products_list, 2);

		test_product_operation_input(browser, products_list);
	...
	'Test DEV-1632 clear cart': function (browser) {
		...
		browser.resizeWindow(1980, 5000);
		...
		for (let i = 0; i < 3; i++) {
			...
			browser.waitForElementVisible('app-cart .cart .cart-head .cart-buttons .button-red', 7000, function () {
				console.log('clear cart button appeared...');
			});
			browser.waitForElementVisible('app-cart .cart .cart-head .cart-buttons .button-red', 7000, function () {
				console.log('clear cart button appeared...');
			});
			browser.click('app-cart .cart .cart-head .cart-buttons .button-red');
			browser.pause(10000);
			browser.waitForElementVisible('app-cart .cart .cart-empty', 7000, function () {
				console.log('empty cart appeared...')
			});
			browser.saveScreenshot('empty.cart.' + i + '.png');
			browser.refresh();
		}
		//end
