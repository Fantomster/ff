...
module.exports = {
	before: function (browser) {
		console.log('Begining test DEV-1635...');
	},

	after: function (browser) {
		console.log('Finishing test DEV-1635...');
		browser.end();
	},

	'Test DEV-1635 order check': function (browser) {
		...
		function test_adding_products_to_cart(b, array, product_count, array_clicks, longDelay) {
			...
			for (let key in array) {
				...
				b.perform(function () {
					console.log('i will make ' + String(array_clicks[i - 1]) + ' clicks now...');
				});
				for (let clickCounter = 0; clickCounter < array_clicks[i - 1]; clickCounter++) {
				...
				let price_for_current_product = parseInt(parseFloat(array[key][1]) * parseFloat(array[key][2]) * array_clicks[i - 1]);
				...
				let productCount = String(parseFloat(array[key][2]) * array_clicks[i - 1]);

				b.verify.attributeEquals('app-product-list table tbody tr:nth-child(' + key + ') td:nth-child(6) input', 'value', productCount);

				b.verify.attributeEquals('app-order .cart-item:nth-child(' + String(cart_key + 1) + ') input', 'value', productCount);
		...
		function test_checkout_products(b, product_list, checkout_size, array_clicks, delay) {
			for (let i = 1; i <= checkout_size; ++i) {
				...
				let current_product_count = String(parseFloat(product_list[String(i)][2]) * array_clicks[i - 1]);
				b.verify.attributeEquals('.checkout-table-goods table tbody tr:nth-child(' + String(i) + ') .table-quantity input', 'value', current_product_count);
				...
				let total_sum = String(parseFloat(product_list[String(i)][1]) * parseFloat(product_list[String(i)][2]) * array_clicks[i - 1]) + ' RUB';
				b.verify.containsText('.checkout-table-goods table tbody tr:nth-child(' + String(i) + ') .table-sum', total_sum);
			}
		}

		function test_order_attribute_notification(b, delay) {
			b.click('.checkout-order .checkout-table-actions a:nth-child(2)', function () {
				console.log('trying to form a checkout...')
			});
			b.waitForElementPresent('app-error-notification .notifications .notification-error .notification-text', delay, true, function (res) {
				console.log('notification appeared...');
			});
			// b.verify.containsText('app-error-notification .notifications .notification-error .notification-text','Укажите
			// дату доставки'); browser.getText("app-error-notification .notifications .notification-error
			// .notification-text", function(result) { this.verify.equal(result.value, "Укажите дату доставки"); });
			b.waitForElementNotVisible('app-error-notification .notifications .notification-error .notification-text', delay, true, function (res) {
				console.log('notification disappeared...');
			});

			test_delivery_date(b, 3, delay);
		...
		let array_clicks = randomArray(products_to_cart);

		let price_sum = test_adding_products_to_cart(browser, products_list, products_to_cart, array_clicks, delay);
		...
		//
		test_checkout_products(browser, products_list, products_to_cart, array_clicks, delay);
		...
		test_order_attribute_notification(browser, delay);
		...
		// });
		//
		// let sum_plus = test_checkout_plus_minus_operation(browser, products_list, products_to_cart, '.quantity-plus',
		// click_count_plus, click_count_minus, delay);  let r_total_plus_sum = new RegExp('^' + String(sum_plus) + '
		// RUB$');  browser.expect.element('app-checkout .checkout-sum
		// strong').text.to.match(r_total_plus_sum).before(delay);  browser.element('css selector', 'body', function () {
		// console.log('testing minus operation...'); });  let sum_minus = test_checkout_plus_minus_operation(browser,
		// products_list, products_to_cart, '.quantity-minus', click_count_plus, click_count_minus, delay);   let
		// r_total_minus_sum = new RegExp('^' + String(sum_minus) + ' RUB$');  browser.expect.element('app-checkout
		// .checkout-sum strong').text.to.match(r_total_minus_sum).before(delay);  browser.element('css selector', 'body',
		// function () { console.log('testing operation with product inputs...'); });  let sum_input =
		// test_checkout_product_input_operation(browser, products_list, products_to_cart);  let r_total_input_sum = new
		// RegExp('^' + String(sum_input) + ' RUB$');  // browser.expect.element('app-checkout .checkout-sum
		// strong').text.to.match(r_total_input_sum).before(delay);  browser.element('css selector', 'body', function () {
		// console.log('testing operation with product comments...'); });  test_checkout_comments(browser,
		// products_to_cart, 'test', delay);  test_delivery_date(browser, days_for_delivery, delay);
