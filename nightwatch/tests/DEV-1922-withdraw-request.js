...
module.exports = {
	...
	'Test DEV-1922 executor appointment': function (browser) {
		...
		browser.waitForElementPresent('.popup-form .popup-form-links a:nth-child(2)', delay);
		browser.getText('.popup-form .popup-form-links a:nth-child(2)', function (result) {
			this.verify.equal(result.value, 'Снять');
		});
		...
		browser.expect.element(supplierAppoint).to.have.attribute('class').which.equals('button').before(delay);
		browser.expect.element(supplierAppoint).text.to.equals('НАЗНАЧИТЬ').before(delay);
